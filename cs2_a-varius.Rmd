---
title: "RIBBIT Analysis for Case Study 2: _Atelopus varius_"
...
audio_folder <- './audio/atelopus_varius_field_data/'

#number of samples per spectrogram window 
spectrogram_window_length <- 32
...
signal_band <- c(1.5,2.5) 

#specify frequency ranges that target confusion species or background noises
noise_bands <- rbind (c(0.0,1.0), #a noise band from 100-1000 Hz
                      c(3.5,4.0) #a second noise band from 1800-2500 Hz (targets Great Plains Toads vocalizations)
                      )

#minimum and maximum Pulse Repetition Rates (PRR) for the target vocalization in pulses/sec
min_PRR = 120 #pulses/sec
max_PRR = 150 #pulses/sec
...
if (save_max_scores_to_file){
  write.csv(max_scores,file.path(audio_folder,'atelopus_varius_top_scores_per_file_r-script.csv'))
}
